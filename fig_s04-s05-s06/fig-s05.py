...
theta  = gr.ReflectionMinimum(4)*1e-3
...
single_pole = residues_green[3]/(k_re - poles_green[3])
zero_loc = np.real(poles_green[3]) + np.imag(poles_green[3])*np.imag(residues_green[3])/np.real(residues_green[3])

print("G0: {}".format(G0))
print('Done!')

### Plot ###
xrange=0.01*detuning_unit_scale/1e3
yrange=300
sc = 0.7
...
ax1 = fig.add_axes([leftMarg, botMarg+3.*(h+vBetweenMarg), w, h],
                  xlabel=r'Depth [nm]', ylabel=r'Field at $\theta_4$')
...
ax2.axvline((np.real(poles_green[3])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1,
...
ax2.legend(fontsize=5, loc=1, handlelength=3)
ax2.get_yaxis().set_label_coords(*ylabel_coords)

### ax3 ###
ax3.axvline(0., color='k', dashes=[1,1], linewidth=1)
ax3.axvline((np.real(poles_green[3])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1)
#ax3.axvline((zero_loc-Fe_resEnergy)*1e3, color='r', dashes=[1,1], linewidth=1, label='predicted CLS zero')
...
ax3.legend(fontsize=5, loc=4, handlelength=3)
...
ax4.axvline((np.real(poles_green[3])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1)
...
ax4.legend(fontsize=5, loc=2, handlelength=3)
...
fig.savefig('fig-s05.pdf') #, dpi=1000
plt.show()
