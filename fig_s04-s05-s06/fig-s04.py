...
eFe57 = pynuss.ResonantElement.fromTemplate('Fe57')
mFe57 = pynuss.Material.fromElement(eFe57)
mSi = pynuss.Material.fromSymbol('Si')

#pynuss Layer structure
nm = 1e-9
#cavity layout -- don't change number of layers
Layers = [pynuss.Layer(mPt,    2.2 * nm),
          pynuss.Layer(mC,    16.0 * nm),
          pynuss.Layer(mFe57,  0.6 * nm),
          pynuss.Layer(mC,    16.0 * nm),
          pynuss.Layer(mPt,   13.0 *nm),
          pynuss.Layer(mSi, -1)]
...
theta  = gr.ReflectionMinimum(1)*1e-3
...
n, d = pynuss_gf.gr_to_NT(gr, mFe57)
n, d = np.array(n), np.array(d)*keV_to_inv_nm

kpar = Fe_resEnergy*np.cos(theta) # [keV]

k_re = np.linspace(kpar, kpar+0.0002, 3000) # [keV]
...
### find poles ###
k_re_pS = np.linspace(kpar,        kpar+0.005, 3000) # [keV]
k_im_pS = np.linspace(    -0.001,       0.001, 3001) # [keV]
...
single_pole = residues_green[0]/(k_re - poles_green[0])
zero_loc = np.real(poles_green[0]) + np.imag(poles_green[0])*np.imag(residues_green[0])/np.real(residues_green[0])
...
xrange=0.003*detuning_unit_scale/1e3
sc = 0.7
...
ax1 = fig.add_axes([leftMarg, botMarg+3.*(h+vBetweenMarg), w, h],
                  xlabel=r'Depth [nm]', ylabel=r'Off-resonant field')
...
### ax1 ###
ax1.plot(Depth0 / nm, Field, 'C0', label=r'field at $\omega_0$')

# Layer boundaries
Thicknesses = pynuss_gf.gr_to_NT(gr, mFe57)[1][1:-1]
print(Thicknesses)
...
ax2.axvline((np.real(poles_green[0])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1,
...
ax3.axvline((np.real(poles_green[0])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1)
...
ax3.legend(fontsize=6, loc=4, handlelength=3)
...
ax4.axvline((np.real(poles_green[0])-Fe_resEnergy)*detuning_unit_scale, color='m', dashes=[2,1], linewidth=1)
...
fig.savefig('fig-s04.pdf') #, dpi=1000
plt.show()
